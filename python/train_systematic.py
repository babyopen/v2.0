...
def run_training_case(case_name, data, test_size=0.2, params=None):
    """
    运行单个训练用例
    ...
    """
    print(f"\n" + "=" * 60)
    print(f"训练用例: {case_name}")
    print("=" * 60)
    
    # 构建特征
    X, y, feature_names = build_features(data)
    ...
    # 预测下一期
    print("\n========== 下一期预测 ==========")
    probabilities = predict_next(model, data.iloc[-1], data)
    ...
    # 保存模型
    model_path = f"zodiac_model_{case_name.replace(' ', '_')}.pkl"
    ...
    results = {
        ...
        'metrics': metrics,
        'top3_recommendations': zodiac_probs[:3],
...
def generate_report(results_list):
    ...
    for result in results_list:
        print(f"{result['case_name']}: {result['data_size']} 条记录")
    
    print("\n二、训练结果对比")
    print("-" * 40)
    print(f"{'用例名称':<20} {'准确率':<10} {'Top-3准确率':<12} {'对数损失':<10} {'推荐生肖'}")
    print("-" * 80)
    for result in results_list:
        case_name = result['case_name']
        accuracy = result['metrics']['accuracy']
        top3_accuracy = result['metrics']['top3_accuracy']
        log_loss = result['metrics']['log_loss']
        recommendation = result['top3_recommendations'][0][1]
        print(f"{case_name:<20} {accuracy:<10.4f} {top3_accuracy:<12.4f} {log_loss:<10.4f} {recommendation}")
    
    print("\n三、模型性能分析")
    print("-" * 40)
    # 分析最佳模型
    best_accuracy = max(results_list, key=lambda x: x['metrics']['accuracy'])
    best_top3 = max(results_list, key=lambda x: x['metrics']['top3_accuracy'])
    best_loss = min(results_list, key=lambda x: x['metrics']['log_loss'])
    
    print(f"最高准确率模型: {best_accuracy['case_name']} ({best_accuracy['metrics']['accuracy']:.4f})")
    print(f"最高Top-3准确率模型: {best_top3['case_name']} ({best_top3['metrics']['top3_accuracy']:.4f})")
    print(f"最低对数损失模型: {best_loss['case_name']} ({best_loss['metrics']['log_loss']:.4f})")
    ...
    print("\n2. 模型方面:")
    print("   - 尝试不同的算法，如XGBoost、LightGBM等")
    print("   - 进行超参数调优，提高模型性能")
...
def main():
    ...
    # 1. 加载完整数据
    df_full = load_data('lottery_history.csv')
    if df_full is None:
    ...
    # 2. 准备训练用例
    training_cases = [
        {'name': '完整数据集', 'data': df_full, 'params': None},
        {'name': '前80期数据', 'data': df_full.iloc[:80], 'params': None},
        {'name': '后80期数据', 'data': df_full.iloc[20:], 'params': None},
        {'name': '完整数据-调整参数', 'data': df_full, 'params': {
            'n_estimators': 300,
            'max_depth': 12,
            'min_samples_split': 4,
            'min_samples_leaf': 2,
    ...
    for case in training_cases:
        result = run_training_case(case['name'], case['data'], params=case['params'])
